            ...
            <div class="taskboard-right-right">
                ...
                <div class="taskwrapper-right">
                    ...
                    <?php
                           ...
                           echo displayTaskRight($saveTasks);                                    
...
<script>
   ...
   //Is This on the right?
        const isOnRight = document.createElement('input');
        isOnRight.type = 'hidden';
        isOnRight.name = 'isOnRight';
        isOnRight.value = '0';
        isOnRight.id =   'amIRight';
        // checker
        ...
        //hidden task
        addTaskForm.appendChild(taskId);
        addTaskForm.appendChild(isOnRight);
        
        
        addTaskForm.appendChild(headTaskDiv);
        addTaskForm.appendChild(descriptionTextarea);
        addTaskForm.appendChild(footTaskDiv);
        
        
        taskDiv.appendChild(addTaskForm);
        
        taskWrap.prepend(taskDiv);
    ...
    taskwrapperRight.addEventListener("drop", function (e) {
        ...
        if (draggedTask && draggedTask.parentElement !== this) {
            ...
            let rightChecker = document.querySelector('.dragging [name="isOnRight"]');
            let idIsNull = document.querySelector('.dragging [name="taskId"]');
            let form = document.querySelector('.dragging [name="taskForm"]');

            rightChecker.setAttribute("value", "1");


            if(idIsNull.value == "null"){
                draggedTask.classList.remove('dragging');

            }else{
            const formData =new FormData(form);
               
                 fetch('includes/task.inc.php',{
                 method: 'POST',
                 body: formData,
    ...
    taskwrapper.addEventListener("drop", function (e) {
        ...
        if (draggedTask && draggedTask.parentElement !== this) {
            ...
            rightChecker.setAttribute("value", "0");
            if(idIsNull.value == "null"){
                draggedTask.classList.remove('dragging');
